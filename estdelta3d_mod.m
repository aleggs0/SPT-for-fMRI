function [objectives] = estdelta3d_mod(XX,Ds,num_compons1)
%ESTDELTA3D Given data, finds cost for various bandwidths
%Set XX=data matrix; if data is fMRI, first apply preprocess.m to the
...
for Dno=1:length(Ds)
    D=Ds(Dno);
    stsum(Dno)=einsum(XX(1:end-D,1:end-D,1:end,:),XX(1+D:end,1+D:end,1:end,:),[1 2 3 4],[1 2 3 4]);
    spt1sum(:,:,Dno)=einsum(XX(:,1:end-D,1:end,:),XX(:,1+D:end,1:end,:),[2 3 4],[2 3 4]);
    spt2sum(:,:,Dno)=einsum(XX(1:end-D,:,1:end,:),XX(1+D:end,:,1:end,:),[1 3 4],[1 3 4]);
...
for m=1:15
    ...
    for Dno=1:length(Ds)
        D=Ds(Dno);
        st=(stsum(Dno)-einsum(XX(1:end-D,1:end-D,1:end,leaveouts),XX(1+D:end,1+D:end,1:end,leaveouts),...
            [1 2 3 4],[1 2 3 4]))/(n-length(leaveouts));
        spt1=(spt1sum(:,:,Dno)-einsum(XX(:,1:end-D,1:end,leaveouts),XX(:,1+D:end,1:end,leaveouts),...
            [2 3 4],[2 3 4]))/(n-length(leaveouts));
        spt2=(spt2sum(:,:,Dno)-einsum(XX(1:end-D,:,1:end,leaveouts),XX(1+D:end,:,1:end,leaveouts),...
        ...
        for tt=15*(m-1)+1:15*m
            ...
            A_matX=einsum(einsum(einsum(XX(:,:,:,tt),c1_est,1,1),c2_est,1,1),c3_est,1,1);
            XCX=einsum(XX(:,:,:,tt),A_matX+B_matX,[1 2 3],[1 2 3]);
            objectives(Dno)=objectives(Dno)-2*XCX/n;
            %diag_objectives(Dno)=diag_objectives(Dno)-2*einsum(XX(:,:,:,tt).^2,(diag(c1_est).*diag(c2_est)'.*permute(diag(c3_est),[3 2 1])+Bsymbol(1,1,1)),[1 2 3],[1 2 3])/210; %n
        end
    end
end
